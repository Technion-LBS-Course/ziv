...
<div style="position:relative">
  ...
  <div id="compare-panel">
    ...
    <div class="legend">
      <span><span class="ld" style="background:#ef4444"></span>Ground route</span>
      <span><span class="ld" style="background:#22c55e"></span>Drive to/from helipad</span>
      <span><span class="ld" style="background:#00d4ff;border-top:2px dashed #00d4ff"></span>Helicopter flight</span>
    </div>
  </div>
  <div id="route-info">
...
var poiData = [
  ...
  {lat:40.7531, lng:-73.9772, name:'Grand Central / Park Ave Corridor', cat:'biz'},
  ...
  {lat:40.7060, lng:-74.0099, name:'Downtown Manhattan Heliport (KJRB)', cat:'heliport'},
...
function flagIcon(color, letter) {
  return L.divIcon({
    html: '<div style="position:relative;width:30px;height:40px">'+
      '<div style="position:absolute;bottom:0;left:4px;width:3px;height:30px;background:'+color+';border-radius:2px"></div>'+
      '<div style="position:absolute;top:0;left:4px;background:'+color+';color:#fff;'+
        'font-weight:800;font-size:12px;padding:2px 6px;border-radius:4px 4px 4px 0;'+
        'box-shadow:0 2px 6px rgba(0,0,0,.4);min-width:18px;text-align:center;'+
        'border:1px solid rgba(255,255,255,.5)">'+letter+'</div>'+
    '</div>',
...
function resetBar() {
  ...
  b.className=''; b.innerHTML='<span style="color:#475569;font-size:12px">Click &#x1F9ED; then pick two points to compare ground vs aerial routes</span>';
}
function setBar(groundMin, airMin) {
  var b = document.getElementById('aerial-bar'), saving = groundMin - airMin;
  if (saving > 0.5) {
...
async function computeMultiModal(pA, pB) {
  ...
  var driveDur = driveR.duration;
  var taxiDur = driveDur * 1.1, transitDur = driveDur * 1.5;
  var walkDist = haversine(pA.lat, pA.lng, pB.lat, pB.lng);
  var walkDur  = walkDist / SPEED_WALK_KMH * 60;
  ...
  if (nearA && nearB) {
    ...
    if (hd > 0.1 && hd <= RANGE_KM) {
      ...
      heli = {
        dur:  d2a.duration + hDur + d2b.duration,
        dist: d2a.dist + hd + d2b.dist,
        hd: hd, hDur: hDur,
        padA: nearA.pad, padB: nearB.pad,
        gA: d2a.geom, gB: d2b.geom
      };
    }
  }

  // best pure-ground time (exclude walking for long trips)
  var groundModes = [{name:'Car', dur:driveDur},{name:'Taxi', dur:taxiDur}];
  if (walkDur < 90) groundModes.push({name:'Walking', dur:walkDur});
  var bestGround = groundModes.reduce(function(a,b){return a.dur<b.dur?a:b;});

  // ── draw RED: best ground (OSRM driving geometry) ─────────────────────────
  if (driveR.geom) {
    routeLayers.push(L.geoJSON(driveR.geom, {
      style:{color:'#ef4444', weight:5, opacity:0.85}
    }).addTo(map));
  } else {
    routeLayers.push(L.polyline([[pA.lat,pA.lng],[pB.lat,pB.lng]],
      {color:'#ef4444', weight:5, opacity:0.7, dashArray:'6 4'}).addTo(map));
  }

  // ── draw GREEN: aerial route (if available) ───────────────────────────────
  if (heli) {
    ...
    routeLayers.push(L.polyline(
      [[heli.padA.lat,heli.padA.lon],[heli.padB.lat,heli.padB.lon]],
      {color:'#00d4ff', weight:3, dashArray:'10 6', opacity:0.95}
    ).addTo(map));
  ...
  var rows = [
    ...
    {name:'&#x1F68C; Transit',   dur:transitDur, dist:driveR.dist,air:'&mdash;', ground:true, note:'est.'},
  ];
  if (heli) rows.push({
    name:'&#x1F697;&#x2708; Car+Heli+Car', dur:heli.dur, dist:heli.dist,
  ...
  rows.forEach(function(r) {
    ...
    var isFastest = Math.abs(r.dur-fastestDur)<0.1;
    var isGBest   = r.ground && Math.abs(r.dur-fastestGround)<0.1;
    tr.className  = r.ground ? (isGBest?'gr-best':'') : 'ai-best';
    var tags='';
    if (isFastest) tags+='<span class="tag tag-f">Fastest</span>';
  ...
  document.getElementById('cp-from-to').textContent=
    'A ('+pA.lat.toFixed(4)+', '+pA.lng.toFixed(4)+')  →  B ('+pB.lat.toFixed(4)+', '+pB.lng.toFixed(4)+')';
...
function findRoute(start, end, allPts) {
  var nodes=[{lat:start.lat,lon:start.lng,name:'Origin'}]
    .concat(allPts).concat([{lat:end.lat,lon:end.lng,name:'Destination'}]);
...
function drawHeliRoute(result) {
  ...
  for(var i=1;i<result.path.length-1;i++)
    routeLayers.push(L.marker([result.path[i].lat,result.path[i].lon],{icon:hIcon('#1565C0'),zIndexOffset:500})
      .bindTooltip(result.path[i].name,{direction:'top'}).addTo(map));
...
APP_PATH = r'c:\Users\ziv.goldenberg\OneDrive - UASC\Documents\Technion\0160833\ziv\app.py'

with open(APP_PATH, encoding='utf-8') as f:
    src = f.read()

START_MARKER = '_ROUTING_HTML_TEMPLATE = """'
# The template ends at </html>""" — find it after the start
start_pos = src.index(START_MARKER)

# Find the closing triple-quote of the template string (</html>""")
CLOSE_MARKER = '</html>"""'
close_pos = src.index(CLOSE_MARKER, start_pos)
end_pos = close_pos + len(CLOSE_MARKER)  # position just after the closing """

# Build replacement: everything up to (not including) the START_MARKER content,
# then new assignment, then the rest of the file from end_pos onward
new_src = src[:start_pos] + NEW_TEMPLATE + src[end_pos:]

with open(APP_PATH, 'w', encoding='utf-8') as f:
    f.write(new_src)

print("Done. Lines written:", new_src.count('\\n'))
