...
<div style="background:#0d2137;border-radius:12px;padding:20px;border-left:4px solid #29b6f6">
  ...
  <p style="color:#90caf9;margin:0 0 14px;text-align:center;font-size:16px">VP, Bank of America</p>
  <table style="width:100%;font-size:15px;color:#e3f2fd;border-collapse:collapse">
    <tr><td style="padding:6px 0"><b>Age</b></td><td>44</td></tr>
    <tr><td style="padding:6px 0"><b>Home</b></td><td>Halesite, NY 11743</td></tr>
    <tr><td style="padding:6px 0"><b>Office</b></td><td>Newport Office Tower, Jersey City, NJ</td></tr>
    <tr><td style="padding:6px 0"><b>Travel</b></td><td>4-5 x /week</td></tr>
    <tr><td style="padding:6px 0"><b>Payment</b></td><td>Corporate Amex - no spending cap</td></tr>
    <tr><td style="padding:6px 0"><b>Priority</b></td><td>Time &gt; Cost</td></tr>
  </table>
  <hr style="border-color:#1e3a5f;margin:14px 0">
  <p style="font-size:18px;font-weight:800;color:#80cbc4;font-style:italic;margin:0;line-height:1.4;text-align:center">
...
<div class="card">
  ...
  <div class="info">
    ...
    <div class="desc">I plan your door-to-door air mobility routes across New York metro —
      ...
      <span style="display:block;margin-top:18px">How can I help you today?</span></div>
    <div class="tags">
...
<div class="leg-row">
  ...
  <div class="leg-card">{card}</div>
    ...
    _MODE_ICON = {"walk": "🚶", "drive": "🚗", "rideshare": "🚗", "helicopter": "🚁"}
    ...
    for bl in legs:
        if bl["mode"] == "helicopter":
            ...
            if waypoints[-1] != _esc(dn):
                # icon for the ground leg preceding this helicopter leg
                pre_mode = legs[0].get("mode", "drive") if bl is legs[0] else "drive"
                for _l in legs:
                    if _l is bl:
                        break
                    pre_mode = _l.get("mode", "drive")
                icons_between.append(_MODE_ICON.get(pre_mode, "🚗")); waypoints.append(_esc(dn))
            icons_between.append("🚁"); waypoints.append(_esc(an))
    if legs:
        last = _esc(legs[-1].get("to","Destination"))[:22]
        if waypoints[-1] != last:
            last_mode = legs[-1].get("mode", "drive")
            icons_between.append(_MODE_ICON.get(last_mode, "🚗")); waypoints.append(last)
